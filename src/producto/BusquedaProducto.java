...
public class BusquedaProducto extends JDialog {
	...
	ProductoAbmProceso model=new ProductoAbmProceso();
	int fila=-1;
	/**
	...
	public BusquedaProducto() {
		...
		table.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent arg0) {
				...
				if(arg0.getKeyCode()==KeyEvent.VK_ENTER){
			        ...
			        if (fila > -1){  			        
			        	ProductoAbm.codigo_producto=String.valueOf(table.getValueAt(fila, 0));
				        ProductoAbm.cod_barra_producto=String.valueOf(table.getValueAt(fila, 1));
				        ProductoAbm.nombre_producto=String.valueOf(table.getValueAt(fila, 2));
				        ProductoAbm.descripcion_producto=String.valueOf(table.getValueAt(fila, 3));
				        ProductoAbm.cantidad_producto=String.valueOf(table.getValueAt(fila, 4));
				        ProductoAbm.precio_producto=String.valueOf(table.getValueAt(fila, 5));
				        ProductoAbm.activar_actualizacion=1;
			            dispose();
			         }
				}else{
		...
		btnBuscar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				Buscar("SELECT id_producto,cod_barra,nombre,descripcion,cant_inicial,precio_venta FROM producto WHERE nombre LIKE '"+textField.getText()+"' '%'");
			}
		});
